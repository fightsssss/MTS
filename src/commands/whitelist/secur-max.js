...
if (db.get(`${message.guild.id}.Owners`) === undefined || db.get(`${message.guild.id}.Owners`) === null) {
    ...
    if (!args.length) {
      return message.channel.send(`${emojis.alert} ${lang.securmax}`)
    }
  if(args[0] === "on") {
  ...
  if(args[0] === "off") {
             ...
             db.set(`${message.guild.id}.antieveryone`, null);
             db.set(`${message.guild.id}.antiChannel`, null);
             db.set(`${message.guild.id}.antibot`, null);
             db.set(`${message.guild.id}.antiRole`, null);
             db.set(`${message.guild.id}.antiWordChoice`, null);
