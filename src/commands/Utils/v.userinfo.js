...
module.exports = {
  ...
  async execute(message, args, client, lang) {
   ...
   // const offline = client.emojis.cache.find((emoji) => emoji.id === '892099238102716436'); // fait
    ...
    embed.addField(`${emojis.user} » Pseudo`, `${user.user.username}`, false)
      .addField(`${emojis.hashtag} » Hashtag `, `#${user.user.discriminator}`, false)
      .addField(`${emojis.id} » ID`, `\`${user.user.id}\``, true)
      ...
      .addField(`${emojis.settings} » Status `, `${status[user.user.presence.status]}`, true)
