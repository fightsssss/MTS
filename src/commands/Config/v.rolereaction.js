...
module.exports = {
    ...
    async execute(message, args, client, lang) {
       ...
       //     .setColor(color)
        //    .setDescription(`${emojis.un}\nChoisir le salon pour la réaction\n${emojis.deux}\nChoisit l'ID du message\n${emojis.trois}\n Choisit la réaction\n ${emojis.quatre}\n Choisit le rôle`)
       //     .setTimestamp()
       ...
       // message.channel.send(embed2)
        ...
        if (message.author.id === message.guild.owner.id || db.get(`${message.guild.id}.Owners`).includes(message.author.id) || message.author.id === owner) {
            ...
            message.lineReply(`${emojis.ticket} ${lang.RoleReactionQuestionChannel}`)
            ...
            message.lineReply(`${emojis.hashtag} ${lang.RoleReactionQuestionMessage}`)
            await message.channel.awaitMessages(filter, {
                    max: 1,
                    time: 120000,
                    errors: ['time']
                ...
                .then(async (collected) => {
                    ...
                    try {
                        ...
                        message.channel.send(`${emojis.no} ${lang.RoleReactionErrorNoMessage}`).then((mssg) => {
                            try {
                                mssg.delete({
                                    timeout: 5000,
                                })
            ...
            message.lineReply(`${emojis.emoji} ${lang.RoleReactionQuestionReaction}`)
            await message.channel.awaitMessages(filter, {
                    max: 1,
                    time: 120000,
                    errors: ['time']
                ...
                .then(async (collected) => {
                    ...
                    if (!emojicheck) {
                        if (!reaction.match(reactionRegex)) {
                            ...
                            message.channel.send(`${emojis.no} ${lang.RoleReactionErrorReaction}`).then((mssg) => {
                                try {
                                    mssg.delete({
                                        timeout: 5000,
                                    })
            ...
            message.lineReply(`${emojis.membre} ${lang.RoleReactionQuestionRole}`)
            await message.channel.awaitMessages(filter, {
                    max: 1,
                    time: 120000,
                    errors: ['time']
                ...
                .then(async (collected) => {
                    ...
                    msg.react(reaction)
                    message.channel.send(`${emojis.yes} ${lang.RoleReactionFinalMessage1} \`${msgid}\`, ${lang.RoleReactionFinalMessage2} ${reaction} ${lang.RoleReactionFinalMessage3} ${role}`).then((mssg) => {
                        try {
                            mssg.delete({
                                timeout: 5000,
                            })
